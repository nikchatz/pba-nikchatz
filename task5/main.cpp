    ...
    const std::vector<int>& aBCFlag)
{
  ...
  for (auto ip = 0; ip < np; ++ip) { // loop over all the point
    ...
    for (auto ipsup = aPsupInd[ip]; ipsup < aPsupInd[ip + 1]; ++ipsup) {
        ...
        // center of gravity = sum of neighbouring points / number of neighbours
        pos_new[0] += aXY[jp * 2 + 0]; // sum the x-coord of neighbouring points
        pos_new[1] += aXY[jp * 2 + 1]; // sum the y-coord of neighbouring points
    }
    // divide by number of neighbours to get center of gravity
